    ...
    public function registration_proccess(Request $request)
    {
        ...
        if (!$valid->passes()) {
            ...
            if ($request->password == $request->password_confirmation) {
                ...
                $res= villages::where(['id' => $request->village])->get();
                
                $pop =  $res['0']->pop;
                $model2 = villages::where('id', $request->village)
                    ->update(['pop' => $pop + 1,
                ]);
                ...
                if ($querry) {
                    ...
                    return response()->json([
                        'status' => 'success', 'msg' => 'You are successfully registered! Please check email for verification.',
